...
🚨 **Critical:** Always verify completion status first, then use appropriate tools - not just explanations!"""
    ...
    async def _generate_pure_code_final_report_with_concise_agents(
        ...
        try:
            ...
            write_operations = 0
            files_created = []
            # Defensive: Only process if history_data is a dict and has 'history' as a key
            if isinstance(history_data, dict) and isinstance(history_data.get("history", None), list):
...
- Total MCP operations: {history_data['total_operations'] if isinstance(history_data, dict) and 'total_operations' in history_data else 0}
