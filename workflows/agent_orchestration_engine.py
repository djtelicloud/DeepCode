...
def get_default_search_server(config_path: str = "mcp_agent.config.yaml"):
    ...
    try:
        if os.path.exists(config_path):
            ...
            default_server = config.get("default_search_server", "google")
            print(f"🔍 Using search server: {default_server}")
            return default_server
        else:
            print(f"⚠️ Config file {config_path} not found, using default: google")
            return "google"
    except Exception as e:
        print(f"⚠️ Error reading config file {config_path}: {e}")
        print("🔍 Falling back to default search server: google")
        return "google"
...
async def github_repo_download(search_result: str, paper_dir: str, logger) -> str:
    ...
    github_download_agent = Agent(
        ...
        instruction="Download github repo to the directory {paper_dir}/code".format(
            paper_dir=paper_dir
        ),
...
async def orchestrate_codebase_intelligence_agent(
    ...
    code_base_path = os.path.join(dir_info["paper_dir"], "code")
...
async def execute_multi_agent_research_pipeline(
    ...
    try:
        ...
        workspace_dir = os.path.join(os.getcwd(), "projects")
...
async def execute_chat_based_planning_pipeline(
    ...
    try:
        ...
        project_name = f"chat_project_{timestamp}"
...
"""
        ...
        markdown_file_path = os.path.join(paper_dir, f"{project_name}.md")
        with open(markdown_file_path, "w", encoding="utf-8") as f:
            f.write(markdown_content)

        print(f"💾 Created chat project workspace: {project_dir}")
